function Set-AFMetaToFansDBStashbox {
    param(
    ...
    do {
        ...
        $originURL = $userConfig.addfriends.stashUrl
    ...
    [string]$originUrl = $userConfig.addfriends.stashUrl
    ...
    # setup.
    if ($userConfig.boxes.fansDbUrl.Length -eq 0) {
        $userConfig = Set-ConfigFansDbStashboxURL -pathToUserConfig $pathToUserConfig
    }

    # Ensure that the target Stash instance can be connected to
    do {
        $StashGQL_Query = 'query version{version{version}}'
        $stashURL = $userConfig.boxes.fansDbUrl
    ...
    # Ensure the Stash URL doesn't have a trailing forward slash
    [string]$stashUrl = $userConfig.boxes.fansDbUrl
    ...
    foreach ($stashScene in $stashScenesToProcess) {
        ...
        # Correct the scene path from meta instance to box instance
        $scenePath = "$($stashScene.files[0].path)".Split("/data/addfriends/")
        $scenePath = "/data/" + $scenePath[1]
